...
# This is all temporary until I can figure out how to access the API Gateway ID in the CF template.
...
for i in "${array[@]}"
do
   aws lambda add-permission --function-name "$i" --statement-id lambda-anony-twit-$(((RANDOM % 10000) + 1)) --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:$REGION:$ACCOUNT_ID:$API_GATEWAY/*/*/*"
done
